"""
应用财务模块第五期数据库迁移脚本
Migration: 20260616_005 - 期末处理与财务报表
"""
...
spec = importlib.util.spec_from_file_location(
    "migration_20260616_005",
    os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                 "migrations", "20260616_005_finance_period_closing.py")
)
...
def apply_migration():
    ...
    if failed_count == 0:
        ...
        print()
        print("创建的表:")
        print("  - period_closing (期末结账表)")
        print("  - financial_report_log (财务报表生成记录表)")
        print()
        print("下一步:")
        print("  1. 运行 python scripts/verify_phase5_integration.py 验证集成")
...
def rollback_migration():
    ...
    print("=" * 80)
    print("回滚迁移: 20260616_005")
    print("=" * 80)
    print()
    print("[WARNING] 这将删除以下表及其所有数据:")
    print("  - period_closing")
    print("  - financial_report_log")
    print()
    ...
    try:
        ...
        # 删除表
        rollback_sql = [
            "DROP TRIGGER IF EXISTS trigger_update_period_closing_updated_at ON period_closing",
            "DROP FUNCTION IF EXISTS update_period_closing_updated_at()",
            "DROP TABLE IF EXISTS financial_report_log",
            "DROP TABLE IF EXISTS period_closing",
        ]
