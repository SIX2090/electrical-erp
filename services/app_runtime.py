...
def create_db_helpers(app, db_config):
    ...
    def pool_key(cursor_factory):
        return "real_dict" if cursor_factory is RealDictCursor else "plain"

    def get_pool(cursor_factory):
        key = pool_key(cursor_factory)
        existing = pools.get(key)
        if existing is not None:
            return existing
        maxconn = int(os.environ.get("PG_POOL_MAX", "8"))
        minconn = int(os.environ.get("PG_POOL_MIN", "1"))
        created = pool.SimpleConnectionPool(
            minconn,
            maxconn,
            host=db_config["host"],
            port=db_config["port"],
    ...
    def get_db(cursor_factory=RealDictCursor):
        ...
        if conn is None or conn.closed:
            conn = get_pool(cursor_factory).getconn()
            setattr(g, cache_key, conn)
        return conn

    @app.teardown_appcontext
    def close_db_connections(exc=None):
        for cache_key, cursor_factory in (
            ("_db_conn_real_dict", RealDictCursor),
            ("_db_conn_plain", None),
        ):
            conn = getattr(g, cache_key, None)
            if conn is None:
            ...
            try:
                ...
                try:
                    get_pool(cursor_factory).putconn(conn)
                except PoolError:
                    pass
    ...
    query_db.__self_get_db__ = get_db
    execute_db.__self_get_db__ = get_db
    execute_and_return.__self_get_db__ = get_db
...
_location_option_cache = {"value": None, "ts": 0}
_location_option_lock = __import__("threading").Lock()


def _read_location_management_enabled():
    """Read the warehouse_location_required system option from the database.

    Returns True if location management is enabled (default), False otherwise.
    The result is cached for 30 seconds to avoid querying on every request.
    """
    import time

    now = time.time()
    with _location_option_lock:
        if _location_option_cache["value"] is not None and (now - _location_option_cache["ts"]) < 30:
            return _location_option_cache["value"]
    ...
    try:
        ...
        if has_app_context():
            ...
            if query_db:
                ...
                val = (row or {}).get("option_value", "1")
                enabled = str(val).strip().lower() in {"1", "true", "yes", "on"}
                if not enabled:
                    location_row = query_db(
                        "SELECT COUNT(*) AS count FROM locations WHERE COALESCE(is_active, TRUE)=TRUE",
                        one=True,
                    )
                    enabled = int((location_row or {}).get("count") or 0) > 0
    except Exception:
        enabled = True

    with _location_option_lock:
        _location_option_cache["value"] = enabled
        _location_option_cache["ts"] = now
    return enabled
...
def register_context_processors(app):
    ...
    def inject_runtime_context():
        ...
        return {
            ...
            "location_management_enabled": _read_location_management_enabled(),
            "topbar_release_info": _read_package_info(),
            "topbar_update_status": get_update_status(),
            "url_with_query": _url_with_query,
        }


def _url_with_query(**overrides):
    """Return current request URL with query params updated by overrides.

    Used by pagination links to preserve filters while changing page/sort.
    """
    args = {}
    try:
    ...
    try:
        ...
        base = request.path if request else "/"
