...
def ensure_system_options_table(execute_db):
    execute_db(
        ...
        CREATE TABLE IF NOT EXISTS system_options (
            ...
            option_key VARCHAR(120) UNIQUE NOT NULL,
            option_value TEXT,
            remark TEXT,
    ...
    try:
        execute_db("ALTER TABLE system_options ADD COLUMN IF NOT EXISTS key VARCHAR(120)")
        execute_db("ALTER TABLE system_options ADD COLUMN IF NOT EXISTS value TEXT")
        execute_db("ALTER TABLE system_options ADD COLUMN IF NOT EXISTS option_key VARCHAR(120)")
        execute_db("ALTER TABLE system_options ADD COLUMN IF NOT EXISTS option_value TEXT")
        execute_db("ALTER TABLE system_options ADD COLUMN IF NOT EXISTS remark TEXT")
        execute_db("UPDATE system_options SET option_key=key WHERE option_key IS NULL AND key IS NOT NULL")
        execute_db("UPDATE system_options SET option_value=value WHERE option_value IS NULL AND value IS NOT NULL")
        execute_db("CREATE UNIQUE INDEX IF NOT EXISTS system_options_option_key_uidx ON system_options(option_key) WHERE option_key IS NOT NULL")
    except Exception:
        pass
...
def ensure_code_rules_table(execute_db):
    ...
    for row in complete_code_rule_rows(default_code_rule_rows()):
        execute_db(
            """
